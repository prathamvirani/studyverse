...
**Phase 07 is accepted. Phase 08 is implemented and awaiting owner review; Phase 09 is not active.** See [coordination](docs/ACTIVE_PHASE.md), [acceptance history](docs/PHASE_HISTORY.md), [advanced RTC architecture](docs/architecture/PHASE_07_RTC.md) and the [future roadmap](docs/phases/README_PHASES.md). Only explicit owner instructions authorize a new phase.

[UMBRELLA_SPEC.md](docs/UMBRELLA_SPEC.md) is the authoritative product/architecture contract. The separately labeled original specification is a historical archive; it is not byte-identical to the amended current specification. [Current architecture](docs/architecture/CODEBASE.md) explains ownership and boundaries; [OAuth setup](docs/OAUTH_SETUP.md) explains provider configuration. `npm run validate:phase08` runs the complete Phase 00–08 regression chain.
...
`npm run validate:phase07` includes the complete Phase 00–06 chain and advanced quality tests. They use synthetic canvas/Web Audio capture but real WebRTC forwarding and separate authenticated test accounts; native screen selection and physical devices are not claimed. The isolated browser authority uses `test-study-` and hostile probes use `probe-study-`, separate from development `study-`. One API authority owns each ephemeral namespace and clears its SFU rooms on restart. See [RTC architecture](docs/architecture/PHASE_06_RTC.md) for permissions, blocks, revocation/replay bounds and operational limits.

More → Media quality provides Basic/Advanced controls, separate send/receive ceilings, Studio Voice, local named profiles and requested/actual diagnostics. Defaults remain conservative. `NUXT_PUBLIC_RTC_QUALITY_STAGE` defaults to `basic`; `a`, `b`, `c`, `d` expose progressively higher request envelopes only for measured experiments. Set it in the web environment (or the existing local Compose environment) and restart the web deployment. Invalid values fall back to basic. These public preferences never change membership, room/source grants or the server admission policy. See [advanced RTC architecture](docs/architecture/PHASE_07_RTC.md) for stage limits and [the report](docs/reports/PHASE_07_REPORT.md) for evidence and rollout limitations.

## Room media

Phase 08 adds the secondary Media dock panel: shared YouTube queue/playback, four original procedural ambience layers, suggestions, owner media permissions and baseline reconciliation. Each listener opens and enables their own visible YouTube workspace tile; media streams directly from YouTube. Closing/minimizing the tile or hiding the page pauses that listener's video. Provider restrictions, ads, buffering and autoplay can affect synchronization.

`ROOM_MEDIA_ENABLED` defaults to true and requires enabled social presence for owner-absence authority. The owner chooses normal controls and, separately, temporary FFA/moderators/preserve permissions while absent. Only the owner saves permanent permissions. Non-owner changes never overwrite the durable baseline; Keep current approves them, Restore mine restores it, and empty rooms reset immediately. Local music/ambience levels, mute and personal ambience use account/device IndexedDB preferences. No personal player, private URL history, provider token or RTC media grant is added.

See [room media architecture](docs/architecture/ROOM_MEDIA.md), [source provenance](packages/features/room-media/provenance.json), and the [Phase 08 report](docs/reports/PHASE_08_REPORT.md).
